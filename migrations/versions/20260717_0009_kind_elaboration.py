...
def upgrade() -> None:
    op.execute("ALTER TABLE app.report_statements DROP CONSTRAINT report_statements_kind_check")
    op.execute(
    ...
    op.execute("UPDATE app.report_statements SET kind = 'elaboration' WHERE kind = 'transition'")


def downgrade() -> None:
    op.execute("UPDATE app.report_statements SET kind = 'transition' WHERE kind = 'elaboration'")
    op.execute("ALTER TABLE app.report_statements DROP CONSTRAINT report_statements_kind_check")
    op.execute(
        "ALTER TABLE app.report_statements ADD CONSTRAINT report_statements_kind_check "

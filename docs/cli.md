...
- **日期**：2026-07-12
- **状态**：待评审
- **关联文档**：`docs/design.md`（系统设计 v3.2，下文引用其章节号）
...
```text
┌─ Research Brief（确认后冻结为本次任务合同）────────────────────┐
│ 问题      评估 2024–2026 年国内 AI Agent 中间件的技术路线与竞争格局 │
│ 时间范围  2024-01 → 2026-07                                     │
│ 地域      中国大陆 + 全球头部对照                                │
│ 必须覆盖  [required] 技术路线对比                                │
│           [required] 商业化进展                                  │
│           [optional] 主要风险                                    │
│ 排除      二级市场投资建议                                       │
│ 来源要求  论文/官方文档/财报/权威媒体 · 每 claim ≥2 独立来源      │
│ 档位      standard（每 worker 工具次数 / Plan 任务数 / replan≤1） │
│ 知识库    industry-reports（14 篇文档）                          │
└─────────────────────────────────────────────────────────────────┘
  [c] 确认并开始   [e] 编辑字段   [r] 重新收敛   [q] 放弃
```

若档位为 `deep`，卡片下方须追加一行预期提示（非阻断）：「深度档最坏可能运行数小时；单次模型/抓取调用仍有超时上限。」依据 m2 §6.1 最坏时长上界。

- **`c` 确认**：Brief 冻结（design §5.1"确认后即合同"），任务进入规划阶段。
- **`e` 编辑**：将 Brief 以 YAML 形式打开 `$EDITOR`；保存退出后经服务端 schema 校验，校验失败显示错误并重新打开编辑器；通过后回到卡片再次确认。选择编辑器而非逐字段 TUI 表单的理由：Brief 是嵌套结构（scope、must_cover 优先级、source_requirements），编辑器表达力足够且实现成本低。
- **`r` 重新收敛**：输入一句修改意向，交回 Scope Agent 重新生成 Brief（不占用澄清反问轮次）。

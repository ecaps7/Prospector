...
- **状态**：实施中（M0 已验收；M1 主图已实现到 Report Verifier、Writer 句级修订与验证后渲染；Job 当前仍以 `draft_rendered` 收口）
...
- 新增 §0.5「Report Verifier」实现记录：逐句分型验证、Claim 权威链持久化、Writer 句级修订回路与验证后确定性引用渲染已接入主图。
- 按当前实现明确边界：每个 revision 全量重验；两次修订后仍失败的句子以 `partial` 保留且不附已验证引用；Job 仍以 `draft_rendered` 收口，正式 `completed` 出口尚未实现。

### v1.7 变更

- Research Verifier 增加 AssertionDisposition（废证）：`assertion_dispositions` 表、source_credibility 硬不变量、usable 投影、Replan 注入 `unusable_assertions`、时间线废证摘要。

### v1.6 变更

- 新增 §0.3「Research Verifier」实现记录：Research Verifier 四项核验、ConflictResolution、Replan 回路、verifier_runs 持久化与事件时间线已完成。
- 回写 `task.started` 事件补充 `subjects` 字段，与 §5.1 事件合同对齐。
...
- M1 **目标架构一次到位**：Planner 决策环、并行通用 Worker、Verifier/Replan、质量门、证据/成文（写作 + 逐句验证）全链路、CLI 闭环，作为**同一交付物**验收。
...
- Report Writer 草稿链与 Report Verifier 已接入后续成文回路，详见 §0.4–§0.5；正式 `completed` 出口仍未实现。
...
- 初稿 revision 1 全量核验；存在失败项时，Report Writer 只接收失败句与允许使用的既有 Excerpt，生成句级 patch 后形成新 revision。当前最多允许两次修订，因此最多核验 revision 1–3；主图当前对每个新 revision 全量重验，`dirty_propagation` 的增量集合尚未接入跨 revision 调度。
- 全部句子通过时进入 `verified`；修订次数用尽仍有失败句时也结束修订并进入渲染，但产物标为 `partial`，保留失败句原文且不给这些句子附已验证引用角标。当前实现没有对失败句执行确定性删除或降格。
...
- 主图已形成 `writer → report_verifier → writer（最多两次修订）/ render → END` 回路，恢复时复用已生成 revision 与已完成 verifier run。渲染后 Report 与 Job 仍记录为 `draft_rendered`，因此这一步是验证后产物出口，不是 M1 目标中的正式 `completed` 出口。
- 已覆盖 Report Verifier 分型判定、前提波次、短 ID 编解码、结构修复、句级 patch、dirty 传播、持久化与主图端到端测试，并保留真实模型 replay/live 测试。
...
2. **没有「未经逐句验证的报告出口」**。正文每句通过 Report Verifier 验证（或被确定性删除/降格）才能交付、Document 快照与 Excerpt 血缘从首次检索起强制——与设计 §11.1 一致，但不因此引入「窄切片交付门」。
...
| 8 | Report Writer 结构化正文（statement id）→ Report Verifier 逐句验证（**evidence + derived**）→ 句级修订环（≤2 轮，触顶确定性删句/降格）→ 确定性引用渲染 |
...
│   ├── statement_prune.py    # 修订环触顶后的确定性删句/降格
...
flowchart TD
  ...
  rver -->|findings：句级修订<br/>增量重验 · ≤2 轮| writer
  rver -->|触顶未过句| prune[确定性删句 / 降格]
  rver -->|全部通过| render[确定性引用渲染]
  prune -->|核心问题仍有回答| render
  prune -->|核心问题失去回答| failGate
  render --> done[完成]
...
- 质量门：完成 / 重大缺口失败（含删句/降格后核心问题失去回答）/ 正文验证失败 / 零证据失败；**预算耗尽不把验证失败变成成功**；无 resolution 的高优先级 contradict 可拦截。
...
- 修订环：句级 findings 打回、换证仅限既有 Excerpt 池（**不补搜**）、statement id 增量重验、硬上限 2 轮；触顶未过句由 `statement_prune` 确定性删除/降格。
- 权威链 `chain_checks` 100%；正文通过验证后文本冻结，引用仅确定性渲染，LLM 不再触碰文本。
...
| 5 | Report Writer + Report Verifier（evidence/derived）+ 修订环/`statement_prune` + citation_render | 成文链一次接完 |
...
| 层 | 内容 |
|----|------|
| Unit | schema、去重、DocumentView source_ids 校验、budget/gates、chain_checks、statement_prune（删句/降格与终止）、planner 线程准入（禁止项不入线程）与 prompt 落 decision_log、摘要 prompt 构造、premise depth |
| Integration（默认 CI） | **完整图** + mock LLM/Exa：并行不串号、Replan、空手 finish、质量门、冲突 resolution、句级修订环（增量重验 + 触顶删句）、SSE 顺序；事件时间线可还原 Planner 派发理由 → Worker 问题 → 搜索/抓取/落证 → Worker 收工 → Planner 下一轮决定 → Verifier/Replan 或结束 |

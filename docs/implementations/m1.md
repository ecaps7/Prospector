...
- `ask` 在 Brief 确认后直接跟随 PG 事件账本；`job events <job_id> --follow` 可回放同一条人类可读时间线（含 Verifier 进入/通过/缺口树/放行）。Planner `finish` 或有证据时决策轮耗尽都会进入 Research Verifier；重大缺口且仍有轮次时 Replan，放行后停在 `outline_pending`。Claim/成文链尚未实现。
...
- 主图（`research_graph.py`）已集成 verifier 节点：Planner `finish`（有证据时）和决策轮耗尽都会进入 Verifier；Verifier `pass` 停在 `outline_pending`；有重大缺口且仍有决策预算时 Replan（major gaps + `unusable_assertions` 注入 Planner 消息线程）；决策轮耗尽则抛出 `VerifierMajorGapError`。
...
- Claim/成文链尚未实现；主图停在 `outline_pending`，等待下一阶段。

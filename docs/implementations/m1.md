...
- 成文链以 `draft_rendered` 为终态；每个 revision 全量逐句验证，触顶失败句保留在 `partial` 产物中且不生成已验证引用角标。

### v1.8 变更

- 新增 §0.5「Report Verifier」实现记录：逐句分型验证、Claim 权威链持久化、Writer 句级修订回路与验证后确定性引用渲染已接入主图。
- 按当前实现明确边界：每个 revision 全量重验；两次修订后仍失败的句子以 `partial` 保留且不附已验证引用；Job 以 `draft_rendered` 收口。

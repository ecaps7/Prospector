...
- **状态**：实施中（M0 已验收；Brief、Planner-Worker 与 Research Verifier/Replan 已完成；当前主图停在 `outline_pending`）
...
- Report Writer 草稿链已实现；Report Verifier、句级修订与正式完成出口仍待下一阶段。

### 0.4 Report Writer 与草稿渲染（已完成）
...
- 确定性 Writer 校验只拦截候选 Excerpt 越界；present_both / adjudicated 的语义履约由 Writer 负责并留给后续 Report Verifier 核对，不以固定句序或物理相邻代替语义判断。
- 草稿、revision 与 statement 已落库；确定性渲染生成 Markdown/JSON 候选引用预览并写入对象存储。主图停在 `draft_rendered`，Job 不进入 completed。

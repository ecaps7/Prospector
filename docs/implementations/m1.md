...
- Writer 输入为冻结 Brief、Plan 历史摘要、全部 usable Assertion 的证据卡、ConflictResolution 与 minor gap；证据卡按收集它的 Task 分组（研究问题 + 该问题下的断言），含 Excerpt ID、Document 来源元数据，以及去重集中、按 `WRITER_EXCERPT_CHAR_LIMIT` 确定性裁剪的 Excerpt 原文。Document 全文仍不进入（D12）。
...
- `ReportVerifierSnapshot` 以当前 revision 的结构化正文为核验对象；每条输入带稳定 `statement_id`、句子性质、候选 Excerpt 原文、前提句与前提深度，成文期不新增搜索或证据。Writer 只校验报告流的格式、候选 Excerpt 和前提的前向引用；推理是否越界、是否最终落到证据由 Report Verifier 判定。超过两层或以前提为无证据展开句的 derived 句记为 `overreach`，进入既有定向修订与 `partial` 出口。

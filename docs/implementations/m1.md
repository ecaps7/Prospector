...
- **版本**：2.0
- **状态**：研究主图已实现。报告子链已迁移为 Research Synthesis → Markdown Writer → Claim
  Attribution → Whole-report Review → deterministic finalization，终态为 `report_rendered`；报告的
  权威合同见 [report-pipeline-refactor.md](../report-pipeline-refactor.md)。本文件余下的旧报告说明是
  迁移历史，不作为当前实现合同。
...
当前 Research Synthesis 将 usable Assertion、Excerpt 和来源信息按 ResearchTask 的研究问题组织后输入强模型。模型上下文使用本次冻结快照内的 `tN / aN / eN / xN` 短引用，分别表示 Task、Assertion、Excerpt 和冲突；模型输出 `assertion_refs / material_conflict_refs`，代码严格按同一命名空间还原为持久化的 `assertion_ids / material_conflict_keys`。一次运行固定先生成分析初稿，再以只含 Brief、初稿、研究任务问题与数量信息、已确认冲突和非来源类 minor gap 的独立消息上下文分别判断核心问题、关系机制、材料取舍和时间边界，并返回分析缺陷；是否采用初稿由代码根据缺陷列表计算。两步原始输出与检查 prompt 一并保存在 `ResearchSynthesisRun.raw_output`，但 Writer、修订 Writer 和 Whole-report Review 只接收最终采用的 `decision / synthesis / reason / evidence_needed`，审计记录不进入后续模型上下文。来源可信度 minor gap 附着到受影响的 finding，使用时在相关表述中落实来源与范围，不再作为全局免责声明输入。最终分析继续使用 `ready / needs_research` 路由；该合同不要求独立的 `falsification` 字段，也不把文风、段落结构或另一种合理写法当成改写理由。
...
- 冻结 Brief 已接入唯一研究图：初始化后进入 Planner，Planner 每轮强制选择扁平的 `dispatch / finish`，全量 prompt、原始输出、结构化决策与拒绝反馈写入 `decision_log`。
- Planner 以 `question + expected_evidence` 自然表达研究任务；`expected_evidence` 描述能够实质回答任务问题的落库事实状态，数量或类别只能描述广度，不能单独构成完成条件。运行时每轮明确提供可用决策、批次容量、Worker 动作/工具、轮次、并行上限、自动抓取数量和 `finish_allowed`。
- Planner 在没有已落库结果时可按 Brief 自由展开；已有结果后，只有尚未解决的问题或新线索值得核查时才继续派发，候选方向未覆盖、还能找到更多资料或重复同类证据本身都不是继续研究的理由。
- 运行时根据 `effort` 注入统一 `max_worker_rounds` 与批次并发上限；Worker 工具调用总数不设上限，单轮最多并行 8 个独立调用。
...
- 每次成功落证后，运行时以任务全部已落库断言独立检查证据目标：`expected_evidence` 是准入下限，goal_met 还要求断言已实质回答任务 `question`；满足时立即收工，否则只把明确缺口回注下一轮。主动收工必须选择 `finish` 动作，运行时记录 `finish_reason`、轮次、工具调用计数与断言摘要。
- 同轮 Worker 并发、任务隔离、恢复幂等、严格动作格式、失败工具调用、轮次耗尽、落证后覆盖判断和事件时间线已有单元/集成测试。数据库迁移已增加 `document_views`、将 `gap_note` 重命名为 `finish_reason`，并删除不再参与执行的 tasks.subjects / research_mode / source_policy。
...
- Research Verifier 在同一节点、同一 verifier run 内执行两段强模型核验。两段共享由冻结快照一次生成的 `tN / aN / eN` 短引用命名空间，模型输出引用合同不含存储 UUID；代码严格反解为领域 UUID 后再执行既有校验与持久化。第一段输出证据资格 review，只判断 Assertion / Excerpt 忠实度、来源承载能力、粒度与冲突，不输出放行结论；代码合并历史与本轮资格状态，生成不含废证和 Excerpt 原文的 usable 投影；第二段输出覆盖 decision，只判断 Plan 形成的可用证据能否实质回应 Brief。
- 第二段先为 Brief 各独立核心要求输出 `answerability_checks`：answered 必须给出有边界答案、承载答案的 usable assertion id 与证据桥梁，blocked 必须说明所缺证据。Pydantic 强制 pass 不含 blocked 项或 major 缺口、needs_research 至少包含一个 blocked 项和 major 缺口、major gap 必须说明 `evidence_needed`；`source_credibility` 缺口必须引用 usable assertion id，重大可信度缺口由代码将相关断言推导为 `unusable`。两段结果合并为既有持久化 `VerifierDecision`，继续记录放行结论、原因、gap、冲突裁决与废证历史。
- 单个任务未完全达到 `expected_evidence` 或候选方向未研究，只有在因此阻断 Brief 核心回答时才构成 major gap。Planner finish reason、Task status 和 Worker goal_met 不构成放行证据；不同方法或平台的材料可以共同形成答案，但相邻对象、替代指标、分别存在的多个机制或无证据连接的解释不能顶替所问关系。
- `ConflictResolution` schema 支持 `present_both` / `adjudicated` 两种终态；模型只点 `assertion_ref`，代码先还原 `assertion_id`，再绑定 `excerpt_ids`；`conflict_key` 由 excerpt_ids 排序哈希生成，幂等去重。
...
- Research Verifier LLM 适配器（`OpenAIResearchVerifier`）两段均使用强模型流式输出；只有 JSON 语法损坏交给不看快照的中档模型修复，判断或引用合同错误回到作出该判断的强模型重答；`VerifierModel` Protocol 支持测试替换。
- Prompt 构造分别由 `research_verifier_messages` 与 `research_coverage_messages` 承担证据资格和覆盖判断；每段只拿完成本职工作所需的快照，完整 prompt 与两段原始输出随同一 verifier run 落库。
...
- Writer 输入为冻结完整 Brief（含 `user_constraints`）、Plan 历史摘要、全部 usable Assertion 的证据卡、ConflictResolution 与 minor gap；证据卡按收集它的 Task 分组（研究问题 + 该问题下的断言），含 Excerpt、Document 来源元数据，以及去重集中、按 `WRITER_EXCERPT_CHAR_LIMIT` 确定性裁剪的 Excerpt 原文。进入模型上下文前，Task / Assertion / Excerpt / 冲突统一转换为本次快照内的 `tN / aN / eN / xN`，不暴露数据库 UUID 和完整冲突哈希。Document 全文仍不进入（D12）。
- 强模型在一次调用内输出 `introduction`、自由组织的 section → paragraph → statement 正文及 `conclusion`；必须回答 Brief 核心问题并遵守非空用户约束，其余内容取舍、结构、详略和表达由 Writer 自行决定。初稿 prompt 只再交代事实与来源不得失真的边界、`source_caveat` 要求和结构化输出协议，不规定写作方法。
- JSONL 中 `paragraph` 表示开始新段落；statement 只输出实际使用的引用字段。`derived` 可直接绑定 Excerpt、绑定此前 statement，或同时绑定两者，但至少需要一种依据；结论依赖的所有关键前文事实必须显式列为 premise，同段相邻不构成隐式依据。
- 确定性 Writer 校验只拦截候选 Excerpt 越界；present_both / adjudicated 的语义履约由 Writer 负责并交给 Report Verifier 核对，不以固定句序或物理相邻代替语义判断。
- 渲染时计算确定性结构指标（事实句/推理句、最长连续事实句、无判断段落数）。Report Verifier 的事实罗列、重复、无判断章节与难以理解的长推理链仍只记 `quality_reminders`；没有回答核心问题或违反用户明确约束则记 `requirement_failures` 并阻止通过。
- 草稿、revision 与 statement 已落库；初稿生成后进入 Report Verifier。逐句失败替换明确失败的 statement；局部整篇质量问题按稳定 paragraph id 原子替换完整段落；中心答案、整体结构或全局约束问题才重写完整报告。

### 0.5 Report Verifier（已完成）

- `ReportVerifierSnapshot` 以当前 revision 的结构化正文为核验对象；每条输入带稳定 `statement_id`、句子性质、候选 Excerpt 原文与来源元数据、前提句，以及研究阶段已确认且与该证据链相关的冲突。Writer 只校验报告流格式、引用存在与前向关系；derived 句可同时持有直接 Excerpt 与前提，两类依据分别写入 ClaimEvidence / ClaimPremise。
- 第一阶段由中档模型按 statement 独立分型验证：`evidence` 核对事实、数字、范围与来源归属，对候选 Excerpt 标注 `support / contradict / partial / irrelevant`；`support` 表示精确支持至少一个关键命题，不要求单独覆盖整句，因此复合句可同时出现 `status=unsupported` 与局部 `relation=support`。`derived` 只读取直接 Excerpt 和显式 premises，检查归纳范围、因果、比较口径与已知冲突；同段未声明句子不补足依据。单轮最多并发 8 个调用，并按前提依赖分波执行；声明前提未通过时下游直接失败。
- Evidence 输入先把候选 Excerpt 的 UUID 映射为 `E1 / E2 / ...` 短代码，模型输出后再还原；`known_conflicts` 里与本句候选重合的条目复用同一短代码，非候选条目只保留文本与 `winning` 标记、不带 id。schema、statement id、kind 由代码校验；`pairs` 多出的非候选条目由代码剥离，缺候选或其余结构错误才进行一次中档模型 JSON 修复，仍不合法时以 `report_verifier_contract_error` 结束 Job。
- `report_verifier_runs` 按 `(report_id, revision, round, attempt)` 持久化 dirty statement 集、findings、逐句判定及失败诊断；合同失败后的恢复创建新 attempt，保留旧失败记录。只有第一阶段形成完整合法的逐句结果后才运行第二阶段；正常逐句失败仍继续整篇检查。第二阶段读取完整报告、逐句结果、usable Assertion 摘要、冲突和 minor gap，检查核心回答、结论支撑、全文一致性、重大遗漏与整体分寸；阻断项写 `requirement_failures`，普通组织问题写 `quality_reminders`。
- 初稿 revision 1 全量核验；逐句失败走定点 statement patch，局部整篇问题走 paragraph patch，只有中心答案或整体结构问题走完整报告重写。三种修订都只能使用既有 Excerpt。初稿与因阶段一失败产生的新 revision 全量重跑“逐句 → 整篇”；上一轮阶段一已全部通过、本轮只为阶段二失败而修订时，复用仍在新稿中的逐句判定，只重跑阶段二。最多允许两次修订，因此最多核验 revision 1–3。
- 全部句子通过时进入 `verified`；修订次数用尽仍有失败句时也结束修订并进入渲染，产物标为 `partial`，保留失败句原文且不给这些句子附已验证引用角标。
- 确定性渲染只从最新完成 verifier run 中状态为 `pass` 且关系为 `support` 的 ClaimEvidence 生成引用；evidence 句与直接绑定 Excerpt 的 derived 句使用同一角标链路。输出 Markdown/JSON 到对象存储，JSON 同时记录 `verification_status`、`failed_statement_ids`、`quality_reminders` 与引用到 Excerpt 的映射。
...
| 2 | Planner 决策环：强制 `dispatch` / `finish`；持久消息线程（随 checkpoint 序列化，准入封闭）；决策日志（含格式错误反馈 + 每轮全量 prompt）；版本化 Plan |
| 3 | 并行通用 Research Worker（自包含任务书）+ 运行时按 effort 注入 `task.budget` + 并发硬闸 |
...
| 8 | Report Writer 结构化正文（statement id）→ Report Verifier 逐句验证（**evidence + derived**）及整份报告履约检查 → 有限修订环（≤2 轮；初稿与句级修订全量重验，阶段二失败后的修订只重跑整篇核验）→ `verified` / `partial` 确定性引用渲染 |
...
│   ├── decisions.py          # dispatch / finish 决策 + 决策日志行
...
│   ├── budget.py             # effort → 批次并发/Worker 轮次；注入 task.budget
│   ├── gates.py
│   └── brief_freeze.py
│
├── flow/
...
| `tasks` | question、expected_evidence；工具与 `max_worker_rounds` 由运行时写入；status、finish_reason |
...
| `decision_log` | dispatch / finish reason、拒绝反馈、格式错误反馈、该轮完整 prompt（回放权威载体，§5.2） |
...
| 事件 | 关键载荷 | 回答的追踪问题 |
|------|----------|----------------|
| `job.phase_changed` | phase、outcome、error_code；进入 `verifier` 时附 plan_version + trigger；成文期附 revision；进入 `cancelling` 时附 requested_via | 整体走到哪一步、取消从哪里发起 |
| `brief.confirmed` | brief_id、effort、confirm_mode（c/e/i） | 研究输入是什么 |
| `planner.started` | decision_round | Planner 是否已经进入本轮模型调用 |
| `planner.decided` | decision_round、decision（dispatch/finish）；dispatch 附 plan_version + task_ids + reason 首行；finish 附 reason 首行 | Planner 计划了什么、怎么想 |
| `planner.rejected` | decision_round、reason_code（over_concurrency / schema_error / empty_finish） | 这轮为何没派活、轮次为何被烧 |
| `task.started` | task_id、question 首行、budget | Worker 在解决什么问题 |
| `task.round_advanced` | task_id、rounds_used、rounds_limit | Worker 当前推进到第几轮 |
...
| `verifier.completed` | verifier_run_id、放行结论、判断理由、major/minor/conflict/废证计数；重大缺口摘要（kind/severity/description/evidence_needed 首行）、冲突摘要（decision/disputed_point 首行）与废证摘要（assertion_id/reason 首行） | 质量门结果 |
...
Web 监控页的“全部”时间线必须按事件 id 完整投影上述业务事件，不截断、不合并、不静默过滤；“关键”只在展示层隐藏高频工具和轮次事件（含 `web_fetch` 403 这类来源不可用，仍属工具噪声）。真正改变研究进程的失败走 `task.finished` / `verifier.completed` / `job.stopped`，不靠工具行文案含「失败」升入关键。每条记录独立显示 `HH:mm:ss`。心跳只维护连接状态，不进入业务时间线。

Job 取消是持久化终态：运行中的 Job 先写 `cancelling`，在当前模型或工具调用结束后的边界，把所有 `pending/running` Task 原子更新为 `cancelled`（`stop_reason=job_cancelled`），再写唯一 `job.stopped(status=cancelled)`。单条 Job 的收尾异常不得结束调度消费循环；调度循环不在运行时，创建 Job 必须失败，不能返回虚假的 `running`。

对称地，**异常中断不是终态**：runner 与调度器对逃出研究图的普通异常只记日志，不写 `job.stopped`——行保持 `running`（合同错误保留图内已落的 `failed` 供诊断）。服务重启不自动恢复，必须由 `prospector-local job resume` 从 checkpoint 续跑；恢复成功后的收尾照常写入唯一 `job.stopped`。终态失败（零证据预算耗尽、Planner 连续格式错误、渲染失败、Verifier 重大缺口且预算耗尽）仍写 `failed` + `job.stopped`；`job resume` 对已写 stopped 的 Job 直接拒绝。
...
research_decisions_used, decision_round_limit, active_task_ids,
...
flowchart TD
  ...
  rver -->|findings：句级修订全量重验；<br/>阶段二修订只重跑整篇 · ≤2 轮| writer
...
动作：每轮以 `response_format=json_object` 输出由内嵌 JSON Schema 描述的扁平判别联合，只能选择 `search / save / finish` 之一；运行时生成 `tool_call_id` 并执行，不接收供应商 Function Calling 参数。
收工：Worker 的 `finish` 只填写 `{action, stop_reason, reason}` 并说明为何无法继续取得必需证据；成功完成只能由 save 后的独立覆盖检查产生。原因持久化为 `finish_reason`，再附断言投影干净摘要（禁止消息轨迹）。

并行：同轮多个 task 独立上下文；单个 Worker 每轮可以并行执行多个彼此独立的工具调用，存在数据依赖的调用必须等待前一轮结果；`produced_by.task_id`、`tool_call_id` 与 span 不得串号。

### 7.3 Planner（§5.2）

二选一 schema；持久 append-only 消息线程（`planner_messages`，随 checkpoint 序列化）：前缀为系统提示 + Brief 快照，逐轮追加决策对象与运行时反馈（断言投影摘要 + 收工声明、拒绝/格式错误反馈、verifier gap、可用决策、具体运行能力和预算余额）；准入封闭（§6.1）。每轮实际发送的完整 prompt 随 `decision_log` 行落库（回放权威载体）。

没有已落库结果时，Planner 可按 Brief 自由展开；已有结果后，继续派发须解决尚未解决的问题或核查新线索。候选方向未覆盖、仍可找到更多资料或重复同类证据，本身都不足以继续派发。

`finish` 前必须重新对照 Brief 核心问题检查所问对象、关系和结果是否已有答案。Worker 的 `goal_met`、`expected_evidence_satisfied`、`done` 与收工理由只表示局部任务状态，不能向上汇总成 Brief 完成；证据直接支持的不确定性或不可分离边界可以构成答案，因此不要求固定研究方法或确定性结论。

硬闸：dispatch 超出 effort 对应的批次并发上限时整批拒绝且仍计轮；空手 finish 拒绝；Planner 决策轮耗尽且零 Excerpt → 失败不进 Verifier。
`budget` 由 `deterministic/budget.py` 按 `effort` 注入，只包含统计每次模型动作的 `max_worker_rounds`。工具调用总数不设上限；单轮并行调用固定最多 8 个，搜索后自动抓取前 2 个结果。

**standard 默认**：Planner 决策轮 12；每批并发 5，Worker 48 轮。完整映射见 §14。

### 7.4 Verifier / 质量门 / 成文

- Verifier：第一段依据 Assertion、Excerpt 和来源元数据核验证据资格、粒度与冲突；代码剔除废证后，第二段仅在 usable 投影上判断 Plan 是否足以回应 Brief，并生成结构化缺口。任务未完全履约只有在阻断核心回答时才构成 major gap；来源可信度不使用 tier 策略表。两段合并为单一 `VerifierDecision` 和既有图出口。
- Research Verifier 出口：重大缺口失败、零证据失败；可披露局限允许进入成文；无 resolution 的高优先级 contradict 可拦截。预算耗尽只停止继续研究，不自动放行。
- 成文（prose-first，设计 §5.4）：Report Writer 以 Brief 核心问题为纲产出带 statement id 的结构化正文（每句标注 evidence/derived/衔接）；Report Verifier 逐句验证——evidence 下钻 Excerpt，derived 同时核对直接 Excerpt 与 ClaimPremise，衔接句核对无事实内容；验证记录落库为 claim（`produced_by: report_verifier`）。
- 修订环：句级 findings 打回、换证仅限既有 Excerpt 池（**不补搜**）、初稿与句级修订全量重验、阶段二失败后的修订只重跑整篇核验、硬上限 2 轮；触顶未过句保留在 `partial` 产物中且不生成已验证引用角标。
- Writer 修订回放：`begin_report_revision` 开写后报告 status 为 `writing`，新 revision 先以 `prompted` 落库。checkpoint 在模型完成前中断时，当前修订仍是 `prompted` 且 revision>1；resume 必须用上一份 `generated` 草稿 + 最新 findings 重放修订 prompt（`bump=False`），不得走初稿 `write` 路径——否则会与已落库的修订 prompt 冲突并拒绝恢复。
...
- `POST /api/jobs`（interactive）、`POST /api/jobs/brief-direct`
- Brief 确认继续（resume：`c` / 编辑后的 Brief / `i` 指令一轮定稿）
- `GET /api/jobs/{id}`、`GET /api/jobs/{id}/events`（SSE + Last-Event-ID，读 PG）
- `GET /api/jobs/{id}/report` 及导出/产物打包
...
| Integration（默认 CI） | **完整图** + mock LLM/Exa：并行不串号、Replan、空手 finish、Research Verifier 出口、冲突 resolution、逐句失败定点修订、整份报告履约失败完整重写、阶段二修订跳过逐句重验与触顶 `partial`、SSE 顺序；事件时间线可还原 Planner 派发理由 → Worker 问题 → 搜索/抓取/落证 → Worker 收工 → Planner 下一轮决定 → Verifier/Replan 或结束 |
...
- [ ] 通过且直接绑定 Excerpt 的 evidence / derived statement→Claim→ClaimEvidence→Excerpt→Document 外键/哈希 100%
...
- [ ] 初稿与因阶段一失败产生的 revision 全量逐句验证并检查整份报告履约；仅阶段二失败触发的修订复用上一轮逐句判定、只重跑整篇核验；修订环 ≤2 轮必然终止
...
1. **`effort →` 硬限制数字表**（`deterministic/budget.py`）：

   | effort | Planner 决策轮 | 每批并发 Worker | 每 Worker 决策轮 |
   |--------|---------------:|------------------:|-------------------:|
   | quick | 8 | 6 | 24 |
   | standard | 12 | 5 | 48 |
   | deep | 24 | 6 | 72 |

   Worker 每轮最多并行 8 个彼此独立的工具调用；搜索后自动抓取前 2 个结果。工具调用总数不设上限，也不写入 `TaskBudget`。

...
- Planner 以 `question + expected_evidence` 自然表达宽搜、深挖、核验、冲突裁决或反例搜索；不再使用人为研究阶段。运行时每轮明确提供可用决策、批次容量、Worker 动作/工具、轮次、并行上限、自动抓取数量和 `finish_allowed`。
...
- Research Verifier 的模型输出只保留 `decision`（pass / needs_research）、`reason`、`gaps`、`conflicts` 与 `assertion_dispositions`。Pydantic 强制 pass 不含 major 缺口、needs_research 至少一个 major 缺口、major gap 必须说明 `evidence_needed`；`source_credibility` 缺口必须引用 `related_assertion_ids`，重大可信度缺口由代码将相关断言推导为 `unusable`。持久化决策继续记录放行结论、原因、gap、冲突裁决与废证历史。
- Research Verifier 要求 Assertion 是单一、可独立核对且忠实于 Excerpt 的陈述；单个任务未完全达到 `expected_evidence` 或候选方向未研究，只有在因此无法实质回应 Brief 时才构成 major gap。
...
- Writer 输入为冻结完整 Brief（含 `user_constraints`）、Plan 历史摘要、全部 usable Assertion 的证据卡、ConflictResolution 与 minor gap；证据卡按收集它的 Task 分组（研究问题 + 该问题下的断言），含 Excerpt ID、Document 来源元数据，以及去重集中、按 `WRITER_EXCERPT_CHAR_LIMIT` 确定性裁剪的 Excerpt 原文。Document 全文仍不进入（D12）。
...
对称地，**异常中断不是终态**：runner 与调度器对逃出研究图的普通异常只记日志，不写 `job.stopped`——行保持 `running`（合同错误保留图内已落的 `failed` 供诊断），由 `prospector-local job resume` 或服务重启从 checkpoint 续跑，恢复成功后的收尾照常写入唯一 `job.stopped`。终态失败（零证据预算耗尽、Planner 连续格式错误、渲染失败、Verifier 重大缺口且预算耗尽）仍写 `failed` + `job.stopped`；`job resume` 对已写 stopped 的 Job 直接拒绝。
...
- Verifier：判断已执行 Plan 形成了哪些可用证据、以 Brief 检查是否足以进入综合，并处理结构化缺口与矛盾（ConflictResolution：present_both / adjudicated）。任务未完全履约只有在阻止实质回应 Brief 时才构成 major gap。Assertion 必须单一、可独立核对且忠实于 Excerpt。来源可信度直接结合 URL、Exa 可用来源元数据、Excerpt 原文和独立佐证判断，不使用来源 tier 策略表。
